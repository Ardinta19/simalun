...
class OrderController extends Controller
{
    ...
    public function driverAction(Request $request, Order $order)
    {
        ...
        // Block final status
        if (in_array($order->status, ['selesai', 'dibatalkan'], true)) {
            return back()->with('error', 'Pesanan sudah final.');
        }

        // Require proof for selesai
        if ($request->status === 'selesai' && !$request->hasFile('proof_image')) {
            return back()->withErrors(['proof_image' => 'Foto bukti wajib di-upload untuk menandai selesai.']);
        }
